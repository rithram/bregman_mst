...
size_t LeftNNSearch<T, TBregmanDiv>::ComputeNeighbor(const Point<T>& query) 
{
  ...
  const T& dist_to_centroid = TBregmanDiv::Divergence(query, tree_->RCenter());
    ...
    const TTreeType* node, const Point<T>& query, const T& dist_to_centroid) 
{
      ...
      && node->Bound().CanPruneRight(query, neighbor_distance_)) 
  {
    ...
    double d_left = TBregmanDiv::Divergence(tree_->Left()->RCenter(), query);
    double d_right = TBregmanDiv::Divergence(tree_->Right()->RCenter(), query);
