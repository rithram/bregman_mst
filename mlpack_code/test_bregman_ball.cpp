...
int main (int argc, char* argv[])
{
  ...
  std::cout << "Testing KL ball.\n";
  {
    ...
    Point<double> q_prime = KLDivergence<double>::Gradient(q);

    // y has a neighbor within 0.001
    bool can_prune_small = kl_ball_small.CanPruneRight(q, q_prime, 0.05);
    //std::cout << "small prune: " << can_prune_small << "\n";
    assert(can_prune_small);
  
    bool can_prune_large = kl_ball_large.CanPruneRight(q, q_prime, 0.05);
  ...
  std::cout << "Testing L2 Ball\n";
  {
    ...
    Point<double> q_prime = L2Divergence<double>::Gradient(q);

    bool can_prune_small = l2_ball_small.CanPruneRight(q, q_prime, 0.03);
    //std::cout << "small prune: " << can_prune_small << "\n";
    assert(can_prune_small);
  
    bool can_prune_large = l2_ball_large.CanPruneRight(q, q_prime, 0.03);
    //std::cout << "large prune: " << can_prune_large << "\n";
    assert(!can_prune_large);
  }
